...
def generate_rss_items(info, hist):
    ...
    for ts, price in reversed(emitted):
        ...
        rss_items.append({
            ...
            'guid': f"{SYMBOL.lower()}-{ts.strftime('%Y%m%d-%H%M')}",
            'pubDate': format_datetime(pubdate_et),
            'description': rss_desc
        })

    return rss_items, now_et
...
def main():
    try:
        ...
        print("Generating RSS items...")
        rss_items, now_et = generate_rss_items(info, hist)
